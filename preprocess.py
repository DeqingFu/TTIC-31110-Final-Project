...
def make_example(seq_len, spec_feat, labels):
    ...
    #feats_list = [tf.train.Feature(float_list=tf.train.FloatList(value=frame)) for frame in spec_feat]
...
def process_data(partition):
    ...
    cnt = 0
    for filename in glob.iglob(partition+'/*.wav.trn', recursive=True):
        with open(filename, 'r') as file:
            cnt += 1
            file_dir = AUDIO_PATH + file.readline()[3:-5]
            print("sub-processing file number", cnt, "in", os.path.basename(partition))
            features_name = file_dir + ".data"
            labels_name = file_dir + ".zh"
            features = np.loadtxt(features_name, delimiter=",")
...
def create_records():
    ...
    for partition in sorted(glob.glob(AUDIO_PATH+'/*')):
        if os.path.basename(partition) in ["data", "README.TXT", "lm_phone", "lm_word"]:
            continue
